...
stringgrid10.enabled:= true;
  ...
  if (cont1<>0) then
       edit9.text:= inputbox('¡ERROR', 'Número Inválido, ingrese un nuevo valor', '')
      else
        begin
      ...
      2: begin
       ...
       5: begin
           Case(Combobox3.ItemIndex) of
              0: begin
                showmessage('Los vectores deben de contener más componentes');
              end;
              1: begin
                ...
                4: begin
                  ...
                  true: begin
                    ...
                    false: begin
                     ...
                     If (beta = 1) then
                         begin
                             Showmessage('Triángulo rectángulo');
                             beta:= 90;
                             Gama:= 180 - (alfa + beta);
                             Edit26.Text:= floattostr(beta);
                             Edit27.text:= floattostr(gama);
                ...
                Case (c1 <> 0) or (c2 <> 0) or (c3 <> 0) or (alfa < 0) or (beta < 0) or (gama < 0) or (beta >179) or (gama>179) or (beta+gama>180) of
                  ...
                  false: begin
                      Case((alfa+beta+gama) <> 180) of
                           true:begin
                             Showmessage('Los ángulos deben sumar 180');
                             memo1.Visible:= false;
                           end;
                        False: begin
                       ...
                       Case (alfa = beta) and (beta = gama) of
                          true: begin
                       Showmessage('Triángulo Equilatero.');
                       While (cont1 = 0) do
                          begin
                          sleep(100);
                          a:= (Random(1000) + 1.00 );
                       ...
                       Case ((alfa = gama) and (beta <> gama)) or ((gama = beta) and (beta <> alfa)) or ((alfa= beta) and (beta<>gama)) of
                        true: begin
                      Showmessage('Triángulo Isósceles.');
                       While (cont1 = 0) do
                          begin
                          sleep(100);
                          a:= (Random(1000) + 1.00 );
                       ...
                       Case (alfa <> beta) and (alfa <> gama) and (gama <> beta) of
                          true: begin
                              showmessage('Triángulo Escaleno');
                              While (cont1 = 0) do
                              begin
                          sleep(100);
                          a:= (Random(1000) + 1.00);
...
begin
     Case(radiogroup11.ItemIndex) of
      ...
      4: begin
           ...
           Case(c1 <> 0) or (c2<>0) or (c3<>0) of
             true: begin
               showmessage('Ingrese la ecuación en forma canónica correctamente.');
             end;
             false: begin
           button42.enabled:= true;
           label32.visible:= true;
             ...
             End;
                false: begin
                     Case radiogroup14.itemindex of
                       -1: begin
                         Showmessage('Debe seleccionar la posición de la elipse.');
                       end;
                        0: begin
                          Edit43.visible:= true;
                          Edit44.visible:= true;
    ...
    Case radiogroup15.ItemIndex of
        0: begin
             ...
             Val(edit50.text,y,c4);
              Case (c1 <> 0) or (c2 <> 0) or (c3 <> 0) or (c4 <> 0) of
                 true: begin
                   showmessage('Valor Inváido');
                 end;
              End;
            Case (h <> x) and (k <> y) of
               true: begin
                   showmessage('Las parábolas rotadas no se calculan.');
               end;
            End;
            Case (h=x) and (k = y) of
               true: begin
                 showmessage('El vértice y el foco no pueden estar en el mismo punto.');
               end;
                 end;
            Case (h = x) and (k <> y) of
               true: begin
        ...
        1: begin
             ...
             Val(edit49.text,directriz,c3);
              ...
              Case radiogroup16.itemindex of
                  -1: begin
                    showmessage('Seleccione la posición de la Parábola');
                  end;
                  0: begin
                      If (H = directriz) then
                          showmessage('Esta no es una parábola')
                        else
                       begin
                        edit51.visible:= true;
                        edit52.visible:= true;
...
Image21.canvas.textout(10,10,'     Parábola: ');
       ...
       0: begin
            ...
            End;
               Case radiogroup19.itemindex of
                 -1: begin
                     showmessage('Seleccione la posición de la Hipérbola');
                 end;
                 0: begin
...
//Fin Asintotas.
//Comienza la hipérbola-Horizonal lado derecho.
...
//Fin de la hipérbola-Horizontal lado derecho.
//Comiezan la hipérbola-Horizontal lado izquierdo.
cont:= 0;
Image22.Canvas.moveto(image22.Width div 2 + trunc(h*5) - trunc(a*5), image22.Height div 2 - trunc(k*5));
While (cont < image15.height) do
   begin;
...
//Fin de la hipérbola-Horizontal lado izquierdo.
//Comienza hipérbola-vertical lado  derecho.
...
//Fin de hipérbola-vertical lado  derecho.
//Comienza hipérbola-vertical lado  izquierdo.
cont:= 0;
Image22.Canvas.moveto(image22.Width div 2 + trunc(h*5), image22.Height div 2 - trunc(k*5)+ trunc(b*5));
While (cont < image15.height) do
   begin;
